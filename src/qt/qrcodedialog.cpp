...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("BEN")
    updateDisplayUnit();
}
...
{
    QString ret = QString("benjamins:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BEN unit input in lnReqAmount, we generate the URI with BEN as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BEN, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
